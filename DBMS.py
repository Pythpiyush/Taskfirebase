...
class Student:
    def __init__(self,root):
        ...
        Addbtn=Button(btn_Frame,text="Add",width=10,command=self
                      .add_student).grid(row=0,column=0,padx=10,pady=10)
        updatebtn=Button(btn_Frame,text="Update",width=10,command=self.update_student).grid(row=0,column=1,padx=10,pady=10)
    ...
    def add_student(self):
        if self.Roll_No_var.get()=="" or self.name_var.get()=="":
            ...
            con=pymysql.connect(host="localhost",user="root",password="Github2020",database="studentinfo")
            cur=con.cursor()
            cur.execute("insert into studenttable values(%s,%s,%s,%s,%s,%s,%s)",(self.Roll_No_var.get(),
                                                                                 self.name_var.get(),
                                                                                 self.email_var.get(),
                                                                                 self.gender_var.get(),
    ...
    def update_student(self):
        con = pymysql.connect(host="localhost", user="root", password="Github2020", database="studentinfo")
        cur = con.cursor()
        cur.execute("update studenttable set name=%s,email=%s,gender=%s,contact=%s,dob=%s,address=%s where roll_no=%s", (
                                                                              self.name_var.get(),
                                                                              self.email_var.get(),
                                                                              self.gender_var.get(),
                                                                              self.contact_var.get(),
    ...
    def delete_data(self):
        con = pymysql.connect(host="localhost", user="root", password="Github2020", database="studentinfo")
        cur = con.cursor()
        cur.execute("delete from studenttable where roll_no=%s",self.Roll_No_var.get())
        con.commit()
        con.close()
        self.fetch_data()
        self.clear()
    ...
    def search_data(self):
        con = pymysql.connect(host="localhost", user="root", password="Github2020", database="studentinfo")
        cur = con.cursor()
        cur.execute("select * from studenttable where "+str(self.search_by.get())+" LIKE '%"+str(self.search_txt.get())+"%'")
        rows = cur.fetchall()
        if len(rows) != 0:
            self.Student_table.delete(*self.Student_table.get_children())
            for row in rows:
